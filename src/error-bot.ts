...
import { getConfig, auth } from './nodebb/api';
...
export async function errorBot() {
	const shuttingDown = new Subject<true>();
	...
	console.log( 'Ready' );
	if( await disposed.toPromise() == null ) { // void if no HMR
		await NEVER.toPromise(); // so wait forever
	}

	shuttingDown.next( true );
